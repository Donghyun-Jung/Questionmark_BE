...
import com.example.duel.alarm.Alarm;
...
import com.example.duel.roadmap.Category;
import com.example.duel.roadmap.Roadmap;
import com.example.duel.roadmap.RoadmapRepository;
import com.example.duel.roadmap.relation.GroupRole;
...
import com.example.duel.step.reference.Reference;
	...
	CommandLineRunner localServerStart(UserRepository userRepository, RoadmapRepository roadmapRepository, StepRepository stepRepository, ReferenceRepository referenceRepository, TilRepository tilRepository, PasswordEncoder passwordEncoder,
		...
		return args -> {
			userRepository.saveAll(Arrays.asList(
					newUser("hayeon@test.com", "ha", passwordEncoder, Role.ROLE_USER),
					newUser("lee@naver.com", "lee", passwordEncoder, Role.ROLE_USER),
					newUser("test@test.com", "test", passwordEncoder, Role.ROLE_USER),
					newUser("admin@test.com", "admin", passwordEncoder, Role.ROLE_ADMIN),
					newUser("hoyai@naver.com", "masterLee", passwordEncoder, Role.ROLE_USER),
			...
			roadmapRepository.saveAll(Arrays.asList(
					...
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "spring boot - 초급편", "듀얼에서 제공하는 spring boot 초급자를 위한 로드맵입니다.", 20, "image.jpg"), //4
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "spring boot - 중급편", "듀얼에서 제공하는 spring boot 중급자를 위한 로드맵입니다.", 30, "image.jpg"),
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "spring boot - 고급편", "듀얼에서 제공하는 spring boot 고급자를 위한 로드맵입니다.", 30, "image.jpg"),
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "Spring JPA - 초급편", "듀얼에서 제공하는 Spring JPA 초급자를 위한 로드맵입니다.", 20, "image.jpg"),
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "Spring JPA - 중급편", "듀얼에서 제공하는 Spring JPA 중급자를 위한 로드맵입니다.", 20, "image.jpg"),
					newDuelRoadmap(User.builder().userId(4L).build(), Category.CATEGORY_DUEL, "Spring JPA - 고급편", "듀얼에서 제공하는 Spring JPA 고급자를 위한 로드맵입니다.", 20, "image.jpg"),

					newGroupRoadmap(User.builder().userId(1L).build(), Category.CATEGORY_GROUP, "JAVA 입문 수업 - 생활 코딩", "생활 코딩님의 로드맵입니다!", true, "dgu12345", true, 10), // 10
					newGroupRoadmap(User.builder().userId(1L).build(), Category.CATEGORY_GROUP, "spring boot 스터디", "같이 spring boot 공부해봐요!", true, "dgu54321", true, 10),
					newGroupRoadmap(User.builder().userId(2L).build(), Category.CATEGORY_GROUP, "JPA 스터디", "박효순 교수님의 JPA를 공부하는 스터디 ^^", false, "hoyai123", false, 20),
					newGroupRoadmap(User.builder().userId(2L).build(), Category.CATEGORY_GROUP, "데이터베이스 스터디", "데이터베이스 공부합시다", true, "hoyoung1", true, 15),
					newGroupRoadmap(User.builder().userId(3L).build(), Category.CATEGORY_GROUP, "융합소프트웨어 1단계", "융합소프트웨어 1단계입니다.", false, "dongguk123", true, 20)
			));
...
//					newUserStepRelation(Step.builder().id(7L).build(), User.builder().id(1L).build(), false)
			...
			referenceRepository.saveAll(Arrays.asList(
					newReference(Step.builder().stepId(4L).build(), "youtube", "https://www.youtube.com/watch?v=Z4OSg3VDGfk"),
					newReference(Step.builder().stepId(4L).build(), "youtube", "https://www.youtube.com/watch?v=PR6hV7scW78"),
					newReference(Step.builder().stepId(6L).build(), "youtube", "https://www.youtube.com/watch?v=DpcJP-wQWGI"),
					newReference(Step.builder().stepId(4L).build(), "web", "https://www.dongguk.edu/main"),
					newReference(Step.builder().stepId(5L).build(), "web", "https://eclass.dongguk.edu"),
					newReference(Step.builder().stepId(5L).build(), "web", "https://itcec.dongguk.edu/main")
			));
	...
	private User newUser(String email, String name, PasswordEncoder passwordEncoder, Role role) {
		return User.builder()
				.email(email)
				.name(name)
				.password(passwordEncoder.encode("leeLEE1!"))
				.role(role)
				.build();
	}
	...
	public Roadmap newDuelRoadmap(User creator, Category category, String name, String description, int stepNum, String image) {
		return Roadmap.builder()
				.creator(creator)
				.category(category)
				.name(name)
