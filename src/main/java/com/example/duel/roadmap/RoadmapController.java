...
public class RoadmapController {
    ...
    // 듀얼 로드맵 생성하기 - 임시 api
    @PostMapping("/roadmaps/duel")
    public ResponseEntity<?> createDuelRoadmap(@RequestBody @Valid RoadmapRequest.CreateDuelRoadmapDTO requestDTO, Errors errors,
                                               @AuthenticationPrincipal CustomUserDetails userDetails){
        RoadmapResponse.CreateRoadmapDTO responseDTO = roadmapService.createDuelRoadmap(requestDTO, userDetails.getUser());
    ...
    // 로드맵 조회하기
    @GetMapping("/roadmaps")
    public ResponseEntity<?> findRoadmapByQuery(@RequestParam(value="category", defaultValue = "duel") String category,
    ...
    // 듀얼 로드맵에 참여 신청하기
    @PostMapping("/roadmaps/duel/{roadmapId}/apply")
    public ResponseEntity<?> applyDuelRoadmap(@PathVariable Long roadmapId, @AuthenticationPrincipal CustomUserDetails userDetails){
        roadmapService.applyDuelRoadmap(roadmapId, userDetails.getUser());
